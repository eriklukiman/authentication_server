...
export class ImageService {
      ...
      .pipe(
        ...
        switchMap((events) =>
          events.length === 0
            ...
            : forkJoin(
                events.map((event) =>
                  this.getEventDetail(clientId, event.name).pipe(
                    catchError(() => this.getEventDetail(clientId, event.id)),
                    map((detail) => ({
                      ...event,
                      location: detail.locations.map((l) => l.mlocName).join(', ') || event.location
                    })),
                    catchError(() => of(event))
                  )
                )
  ...
  private getEventPhotos(clientId: string, eventName: string, options: ImageQueryOptions): Observable<ImageItem[]> {
